...
export default function Schedule() {
  ...
  return (
    <section className="py-10 md:py-16 bg-zinc-950 relative overflow-hidden" id="schedule">
      {/* Glow */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[700px] h-[300px] bg-[#F40C41]/6 blur-[100px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col items-center text-center gap-4 mb-12">
          ...
          >
            CLASS <span className="text-[#F40C41]">SCHEDULE</span>
          </h2>
          ...
          {days.map((day) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full font-black uppercase tracking-widest text-xs transition-all duration-200 ${
                activeDay === day
                  ? "bg-[#F40C41] text-white shadow-[0_0_20px_rgba(244,12,65,0.4)]"
        ...
        >
          {schedule[activeDay].map((cls, i) => (
            <motion.div
              ...
              className="group flex items-center gap-4 p-4 rounded-2xl border border-white/6 bg-white/2 hover:border-[#F40C41]/30 hover:bg-[#F40C41]/4 transition-all duration-200"
            >
              <div className="text-center min-w-[64px]">
                <p className="text-[#F40C41] font-black text-sm">{cls.time}</p>
                <p className="text-white/25 text-[10px] font-bold uppercase tracking-widest">{cls.coach}</p>
              </div>
              <div className="w-px h-10 bg-white/8 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-white font-black uppercase tracking-tight text-sm">{cls.name}</p>
                <div className="flex items-center gap-1 mt-0.5">
                  <Users className="w-3 h-3 text-white/25" />
                  <span className="text-white/25 text-[10px] font-bold">{cls.spots} spots left</span>
                </div>
              </div>
            </motion.div>
          ))}
